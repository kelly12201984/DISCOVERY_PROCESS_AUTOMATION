-- ============================================================
-- PO STATUS BY JOB (DETAIL) — V4 FINAL
-- Joins through the SOURCE table (PO_Detail + Material_Req bridge)
--
...
SELECT
    ...
    -- PO info (joined via Source table)
    ph.PO AS PO_Number,
    ...
    -- Source table receipt info
    src.Act_Qty AS Source_Recv_Qty,
    src.Last_Recv_Date,
    ...
    CASE
        ...
        WHEN ph.PO IS NOT NULL THEN 'ON ORDER - Due '
            + CONVERT(VARCHAR(10), pd.Due_Date, 101)
        WHEN mr.Status = 'C' AND ISNULL(mr.Act_Qty, 0) >= mr.Est_Qty THEN 'RECEIVED (no PO link)'
        WHEN mr.Status = 'C' THEN 'CLOSED'
        ELSE 'NO PO CREATED'
